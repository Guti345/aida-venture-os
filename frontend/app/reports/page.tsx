...
import { lpReport, portfolioSnapshot } from '@/lib/mock/reports'
...
export default function ReportsPage() {
  const snapshotRows = portfolioSnapshot.map((s) => [
  ...
  return (
    <PageWrapper>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard label="Fondo" value={lpReport.fund_name.split(' ').slice(-2).join(' ')} />
        <KPICard label="Total invertido" value={fmtUSD(lpReport.total_invested_usd)} />
        <KPICard
          label="MOIC actual"
          value={`${lpReport.fund_moic_current.toFixed(2)}x`}
          trend="up"
        />
        <KPICard
          label="IRR estimado"
          value={`${lpReport.fund_irr_estimate_pct.toFixed(1)}%`}
          trend="up"
        />
      </div>
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs text-[#9CA3AF]">
                {new Date(lpReport.report_date).toLocaleDateString('es-CO', {
                  year: 'numeric', month: 'long', day: 'numeric',
                })}
              </span>
            </div>
            <p className="text-sm text-[#0A0B0E] leading-relaxed">
              {lpReport.narrative_summary}
            </p>
